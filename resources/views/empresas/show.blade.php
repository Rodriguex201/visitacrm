    ...
    <section class="space-y-4 pb-24" x-data="historialVisitas()" x-init="init()">
        ...
        <div class="grid gap-4 lg:grid-cols-2 lg:items-start">
            ...
            >
                    ...
                    <div class="absolute right-3 top-3 flex items-center gap-2">
                        @if ((auth()->user()?->tipo_usuario ?? null) === 'administracion')
                            <button
                                ...
                                class="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100 hover:text-slate-900"
                                title="Cambiar referido"
                            >
                                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17 21l-5-5 5-5" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M7 3l5 5-5 5" />
                                </svg>
                                <span>Cambiar referido</span>
                            </button>
                        @endif
                        ...
                        <button
                            ...
                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:bg-slate-100 hover:text-slate-900"
                ...
                <div class="space-y-3 pr-10 md:pr-44 text-slate-600">
        ...
        <article class="space-y-6 rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div x-cloak x-show="showUsuarioModal" class="fixed inset-0 z-50 flex items-center justify-center px-4" @keydown.escape.window="cerrarModalUsuario()">
                ...
                <div class="relative z-10 w-full max-w-xl rounded-xl bg-white p-5 shadow-xl">
                    <div class="mb-4 flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-slate-900">Cambiar referido</h3>
                    ...
                    <div class="space-y-3">
                        <div class="flex gap-2">
                            <input type="text" x-model="usuarioQuery" @input.debounce.300ms="buscarUsuarios()" placeholder="Buscar por código, nombre, teléfono o tipo" class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                        ...
                        <div class="max-h-64 overflow-y-auto rounded-lg border border-slate-200">
                            <template x-if="usuarioLoading">
                                <p class="px-3 py-2 text-sm text-slate-500">Buscando usuarios activos...</p>
                            </template>
                            ...
                            <template x-for="usuario in usuariosResultados" :key="usuario.id">
                                <button
                                    type="button"
                                    @click="usuarioSeleccionadoId = usuario.id"
                                    class="flex w-full items-start justify-between border-b border-slate-100 px-3 py-2 text-left transition hover:bg-slate-50"
                                    :class="usuarioSeleccionadoId === usuario.id ? 'bg-blue-50' : ''"
                                >
                                    <div>
                                        <span class="block text-sm font-medium text-slate-700" x-text="`${usuario.codigo || 'S/C'} - ${usuario.name || usuario.nombre || 'Sin nombre'} - ${usuario.tipo_usuario || 'sin tipo'}`"></span>
                                        <span class="text-xs text-slate-500" x-text="usuario.telefono || 'Sin teléfono'"></span>
                                    </div>

                                    <span
                                        x-show="usuarioSeleccionadoId === usuario.id"
                        ...
                        <div class="flex justify-end gap-2 pt-2">
                            ...
                            <button type="button" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60" @click="guardarUsuarioAsignado(usuarioSeleccionadoId)" :disabled="usuarioSaving || !usuarioSeleccionadoId || usuarioSeleccionadoId === empresaUser?.id">
