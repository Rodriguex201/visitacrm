...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-gray-50 font-sans antialiased text-slate-900">
        <div class="min-h-screen">
            <aside class="hidden md:fixed md:inset-y-0 md:flex md:w-[260px] md:flex-col md:border-r md:border-slate-200 md:bg-white">
                ...
                <div class="border-t border-slate-200 px-3 py-4">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-slate-600 transition hover:bg-slate-100 hover:text-slate-800">
                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" aria-hidden="true">
            ...
            <main class="min-h-screen pb-24 md:pb-6 md:pl-[260px]">
                <div class="mx-auto w-full max-w-[1300px] px-4 py-4 md:px-5 md:py-5">
        ...
        <nav class="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white px-2 py-1.5 md:hidden">
            <div class="grid grid-cols-6 gap-1">
                ...
                <form method="POST" action="{{ route('logout') }}" class="flex">
                    ...
                    <button type="submit" class="flex w-full flex-col items-center justify-center gap-1 rounded-lg px-1 py-0.5 text-[11px] font-medium text-slate-500">
                        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" aria-hidden="true">
