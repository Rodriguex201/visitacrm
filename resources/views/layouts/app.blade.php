...
<link rel="icon" type="image/x-icon" href="{{ asset('favicon.ico') }}?v=99">
<link rel="shortcut icon" type="image/x-icon" href="{{ asset('favicon.ico') }}?v=99">
<link rel="icon" type="image/png" sizes="32x32" href="{{ asset('favicon.png') }}?v=99">
<link rel="icon" type="image/png" sizes="16x16" href="{{ asset('favicon.png') }}?v=99">
    ...
    >
        ...
        @php
            ...
            $isHerramientasDisponibles = request()->routeIs('herramientas.*');
        ...
        <div class="min-h-screen md:flex">
            ...
            >
                <div class="flex items-start justify-between border-b border-slate-200 px-5 py-5" :class="sidebarCollapsed ? 'md:px-3' : ''">
                    <div x-show="shouldShowLabels()" x-transition class="md:block">
                        @if ($sidebarLogoUrl)
                            ...
                            <div class="flex h-[4.5rem] w-full max-w-[252px] items-center justify-start overflow-hidden rounded-lg">
                ...
                <nav class="flex-1 space-y-1.5 px-3 py-4 text-sm" :class="sidebarCollapsed ? 'md:px-2' : ''">
                    ...
                    <div
                        x-data="{ toolsOpen: @js($isHerramientasDisponibles) }"
                        class="space-y-1"
                    >
                        <button
                            type="button"
                            class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left font-semibold transition {{ $isHerramientasDisponibles ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700' }}"
                        ...
                        >
                            ...
                            <span x-show="shouldShowLabels()" x-transition class="flex flex-1 items-center justify-between gap-2">
                                <span>Herramientas disponibles</span>
                                <svg class="h-4 w-4 transition-transform duration-200" :class="toolsOpen ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                        ...
                        <div
                            x-show="toolsOpen && shouldShowLabels()"
                            x-transition.origin.top.duration.200ms
                            x-cloak
                            class="space-y-1 pl-10"
                        >
                            <a
                                href="{{ route('herramientas.redes') }}"
                                class="block rounded-lg px-3 py-2 font-medium transition {{ $isHerramientasRedes ? 'bg-blue-100 text-blue-700' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700' }}"
                                @click="sidebarOpen = false"
                            >
                                Redes
                            </a>
                            <a
                                href="{{ route('herramientas.ofrecer') }}"
                                class="block rounded-lg px-3 py-2 font-medium transition {{ $isHerramientasOfrecer ? 'bg-blue-100 text-blue-700' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700' }}"
                                @click="sidebarOpen = false"
                            >
                                Qué puedo ofrecer
                            </a>
